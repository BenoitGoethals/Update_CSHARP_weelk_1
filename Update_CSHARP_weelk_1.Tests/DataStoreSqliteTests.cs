...
public sealed class DataStoreSqliteTests : IDisposable
{
    private readonly string _tempDir;
    private readonly string _dbPath;

    public DataStoreSqliteTests()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "sqlite-tests-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(_tempDir);
        _dbPath = Path.Combine(_tempDir, "books.db");
    }
    ...
    private DataStoreSqlite CreateStore() => new(_dbPath);
    ...
    [Fact]
    public async Task ReadAllAsync_WhenEmpty_ReturnsEmpty()
    {
